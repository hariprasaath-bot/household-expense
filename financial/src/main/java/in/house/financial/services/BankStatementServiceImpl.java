...
public class BankStatementServiceImpl implements BankStatementService {
    ...
    public SseEmitter parseUploadedStatementFile(String bank, MultipartFile uploadedStatementFile, SseEmitter emitter) throws IOException {
        String progress = "Started processing the uploaded statement file";
        Integer percentage = 5;
        UploadProgress uploadProgress = new UploadProgress(progress, percentage);
        emitter.send(uploadProgress);
        ...
        uploadProgress.setCurrentProgress("Completed parsing the uploaded statement file -- no of records: " + parsedStatement.size());
        uploadProgress.setStatus(uploadProgress.getStatus() + 25);
        emitter.send(uploadProgress);
        List<TransactionRecord> enrichedRecords = statementRecordEnrich.enrich(parsedStatement);
        uploadProgress.setCurrentProgress("Completed enriching the parsed statement file -- no of records: " + enrichedRecords.size());
        uploadProgress.setStatus(uploadProgress.getStatus() + 50);
        mongoConnector.putDataInMongo(enrichedRecords, "transaction_records");
        uploadProgress.setCurrentProgress("Completed storing the enriched statement file in mongo");
        uploadProgress.setStatus(uploadProgress.getStatus() + 20);
        return emitter;
    }
